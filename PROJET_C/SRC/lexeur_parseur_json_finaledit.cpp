...
cout<< "state 8"<<endl;
              switch (c2.gettype())
              {
                case NOM:u2=u2+1;
                        ...
                        if (readytosave2==1)//
                          {
                            //name of var good so we can save it
                            vnomjson.push_back(c2.getname());
                          }
...
cout<< "state 12"<<endl;
              switch (c2.gettype())
              {
                case NOMBRE: cout<<"im juste here"<<endl;
                              valeur = c2.getname();// on recupere la chaine dans un string
                              cout <<c2.getname();
                             taillevaleur = valeur.size();// on recupere la taille de la chaines
                             cout << taillevaleur <<endl;
                             ...
                             do
                             {
                               ...
                               {
                                 vvaleurjson.push_back(chiffre);// on stocke le dernier bool en memoire
                               }
                               if (valeur[p]== '\'')
                               {
                                 //on ne fait rien
...
cout<< "state 16"<<endl;
            ...
            case ERREUR:cout<< "je suis dans une erreur et je sors"<<endl;
              ...
              switch (erreur2)
              {
                ...
                case 6:cout <<"erreur6: on attend le caractère '{' et on a recu "<< c2.getname()<<" à la ligne "<< c2.getline() << endl;
                break;
    ...
    vector<bool>::iterator itr3;
    for(itr3 = vvaleurjson.begin() ; itr3 != vvaleurjson.end() ; ++itr3)
    {
      cout<<*itr3<<endl;
    }

    vector<string>::iterator itr4;
    for(itr4 = vnomjson.begin() ; itr4 != vnomjson.end() ; ++itr4)
    {
      cout<<*itr4<<endl;
    }
